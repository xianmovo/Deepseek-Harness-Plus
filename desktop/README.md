...
> `desktop/` 下执行 `npm approve-scripts` 批准后重新安装依赖。
...
- **不携带测试凭据**：仓库与打包产物中没有任何 API Key；本机测试用的
  `DEEPSEEK_API_KEY` 已从配置中删除。`desktop/.gitignore` 忽略
...
- **打包范围**：`electron-builder.yml` 只打包 `src/`、运行时依赖与品牌
  ...
  安装包。
- **打包**：
  - 命令：`cd desktop && npx electron-builder --win --x64`（首次需 `npm i -D electron-builder`；
    证书受限环境加 `$env:NODE_OPTIONS="--use-system-ca"`，下载工具用
    `$env:ELECTRON_BUILDER_BINARIES_MIRROR="https://npmmirror.com/mirrors/electron-builder-binaries/"`）。
  - 输出到 `E:\DeepseekWorkSpace\DSHP\_win64`（`win-unpacked\` 绿色目录 + NSIS 安装包）。
...
- **零侵入**：`desktop/` 是独立 npm 项目，不在仓库 pnpm workspace 里，不触碰上游配置与构建门禁。
